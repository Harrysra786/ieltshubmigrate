...
*/
  ...
  if (user) {
    email = user.email;
    const markingsRef = doc(db, "markings", email); // Get a reference to the document
    const essayDocRef = doc(markingsRef, "essaymarking", "essay3"); // Get a reference to the 'essay3' document in the subcollection
    const docSnap = await getDoc(essayDocRef); // Fetch the document
...
<div class="mt-2 border-2 rounded border-neutral-800 overflow-clip bg-[#F8F0E3] ">
  ...
  <div id="comparison" class="px-6 py-4 topic bg-[#F8F0E3] s2">
    ...
    <TextDiff Essay={essayresponse} Correction={gral} />
    {/if}
  </div>
</div>
...
<div class="mt-2 border-2 rounded border-neutral-800 overflow-clip bg-[#F8F0E3] ">
  ...
  <div id="correction" class="px-6 py-4 topic bg-[#F8F0E3] s2">
    {@html gral}
      ...
      <div class="mt-2 border-2 rounded border-neutral-800 overflow-clip bg-[#F8F0E3] "><div class="flex flex-row px-2 pt-3 text-base topic-Info justify-evenly bg-[#F8F0E3]"></div> <div class="px-6 py-4 topic bg-[#F8F0E3] s2"><SvelteMarkdown source={ebs} /></div></div>
