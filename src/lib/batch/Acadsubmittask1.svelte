<script>
  ...
  $: wordCount = graph.split(" ").length - 1;
  ...
  async function sendData() {
    ...
    const data = { 
      Graph: graph, // use the letter as the Letter
    ...
    const querySnapshot = await getDocs(collection(db, "markings", email, "graphmarking"));
    ...
    await setDoc(doc(db, "markings", email, "graphmarking", `graph${docCount + 1}`), data);
    submitStatus = 'sent';
  } catch (error) {
...
{/if}
    ...
    <div class="flex-col self-end w-11/12">
      <form class="flex w-11/12">
        ...
        <textarea id="graph" bind:value={graph} class="w-11/12 p-4 mt-1 mb-1 bg-white border-2 border-solid rounded resize-y border-neutral-800" placeholder="Type your Graph here..."></textarea>
