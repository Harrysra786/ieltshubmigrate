<script>
  ...
  import { sendMessageToSavedMessages } from "../../telegramService";
  import { _chatbot } from '../../../../src/routes/api/askgpt4/+server.ts';
...
function autoResize(element) {
  ...
  try {
    ...
    setTimeout(() => alert("Essay Submitted successfully!"), 0);
...
async function processGpt4Response(nameInput, textInput) {
  try {
    const [response1, response2] = await Promise.all([
      _chatbot.generateResponse(textInput, "Correct any grammar mistakes in the following essay:"),
      _chatbot.generateResponse(textInput, "What score would this essay get in IELTS writing?"),
    ]);
