<script>
    ...
    attributes: [
      { icon: '/img/Hardev Sir Avatar.png', text: 'Writing' },
      { icon: '/img/Hardev Sir Avatar.png', text: '110 Pages' },
    ],
    rating: '⭐ 4.8 (10)',
    price: '🧑🏼‍🎓₹60',
  });
      ...
      <article class="grid m-3 border-black shadow-pop-br">
        <a href="/samplebook">
          ...
          <div class="border-black border-[1px] font-mono text-base text-center bg-white">
            <header>
              <h3 class="flex justify-center font-bold">{product.title}</h3>
              <div class="flex border-y-[1px] border-black justify-evenly items-center"> <!-- added items-center -->
                {#each product.attributes as attribute}
                  <h3 class="font-sans text-xs divide-black text-zinc-700">
                    <img src="{attribute.icon}" alt="Icon" class="inline-block w-4 h-4 mr-1"> <!-- replaced emoji with img tag -->
                    {attribute.text}
                  </h3>
                {/each}
              </div>
