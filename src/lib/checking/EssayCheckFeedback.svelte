    ...
    if (selectedEmail) {
      ...
      buttons = await Promise.all(essaynums.map(async (essaynum) => {
        ...
        if (docSnap.exists()) {
          ...
          let tenHoursInMilliseconds = 10 * 60 * 60 * 1000;
          return {
            label: essaynum,
            enabled: currentTime - submissionTime >= tenHoursInMilliseconds
          };
        }
        return { label: essaynum, enabled: false };
      }));
...
<div id="review"
      class="flex flex-col items-center px-10 pb-6 bg-white border-solid rounded-sm border-neutral-900 lg:min-w-[840px] max-w-[840px] border-x-2 border-b-2"
    >
