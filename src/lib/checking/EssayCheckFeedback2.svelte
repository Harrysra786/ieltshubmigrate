...
onMount(async () => {
  ...
  // Fetch emails
  const emailQuerySnapshot = await getDocs(collection(db, 'essaymarkings')); // Changed from 'markings' to 'essaymarkings'
...
async function fetchData() {
  loading = true;
  if (email && essaynum) {
    const docRef = doc(db, 'essaymarkings', email, 'submissions', 'essay' + essaynum);
...
async function fetchDropdownData(selectedEmail) {
  try {
    ...
    if (selectedEmail) {
      const essayQuerySnapshot = await getDocs(collection(db, 'essaymarkings', selectedEmail, 'submissions')); // Changed from 'markings' to 'essaymarkings'
