<script>
    ...
    import { essayTopic, essayNumber } from '$lib/store'; // import the store
    let imageSrc = ' '
    let imageSrc2 = ' '
    let essays = [];
    let currentIndex = 0;
...
<section class="flex-row items-center justify-center my-8 bg-white">
  ...
  <div class="w-2/4 mb-2"><div class="relative">
    <p class="text-center text-xs max-w-[800px] mt-2 px-5 font-semibold">Search the essay topic</p>
    <input 
      bind:value={searchTerm} 
      on:input={debouncedSearch} 
      class="w-full px-3 py-2 mt-2 text-xs leading-tight text-center text-gray-700 border rounded shadow appearance-none border-bg-neutral focus:outline-none focus:shadow-outline" 
      placeholder="Search Essay Topic" 
      autocomplete="off"
    />
    <div class="absolute z-50 w-full mt-2 text-xs bg-white border border-gray-200 rounded shadow">
      {#each searchResults as result (result.essaynumber)}
        <a 
          ...
          class="block px-4 py-2 text-xs border border-gray-700 border-dashed hover:bg-gray-200"
        >
          {result.essaytopic}
        </a>
      {/each}
      {#if searchResults.length === 10}
        <a href="#" on:click|preventDefault={showMoreResults} class="block px-4 py-2 text-xs hover:bg-gray-200">See more results</a>
      {/if}
      {#if searchResults.length === 0 && searchTerm !== ''}
        <div class="block px-4 py-2 text-xs">No essay topic matches your query!</div>
      {/if}
    </div>
  </div>
  </div></div>
  <img class="block w-10 h-10 mx-auto mt-8" src="/img/arrowdown.svg" alt="clock" />
<div class="flex items-center justify-center mt-1">
    <div><p class="text-center text-xs max-w-[800px] mt-2 px-5 font-semibold">Or, Choose by Essay Type</p></div>
</div>
<div class="flex items-center justify-center">
  <div class="flex flex-wrap items-center justify-center mx-2 max-w-[500px]">
...
<div class="flex items-center justify-center mt-1">
  <div>
  <p class="text-center text-xs max-w-[800px] mt-2 px-5 font-semibold">Or, choose the topic area</p>
  </div>
</div>
<div class="flex items-center justify-center">
  <div class="flex flex-col items-center justify-center mx-2 max-w-[500px]">
  ...
  <div id="viewport" class="flex items-center justify-center px-2 mt-4">

    
    <div class="max-w-[900px] mx-5 px-12 tiny:px-3 tiny:mx-1 py-2 bg-zinc-100 shadow-black ">
        
                  <p class="pt-3 mt-2 text-lg font-bold tracking-wider text-center">Topic</p>
                  <div class="flex flex-row items-center">
                  <a class="mr-4" href="" on:click|preventDefault={prevEssay}>
                    <img
                    src="/img/doublearrowleft.svg"
                    class="w-8 h-8 imgshadowed"
                    alt="Icon"
                    />
                </a>
                  <div class="border-2 rounded-sm border-neutral-800 overflow-clip">
                      <div class="flex flex-col tiny3:flex-row tiny3:items-center px-2 py-3 text-base topic-Info justify-evenly bg-[#F1E0C5] border-b-neutral-800 border-2 overflow-clip">
                          <div class="flex flex-row items-end">
                            <img
  src={imageSrc}
  class="w-6 h-6 mr-2 imgshadowed"
  alt="Icon"
/>

                              <p class="text-sm font-bold"> {essays[currentIndex]?.essaytype}
                              </p>
...
/>
                              ...
                              <p class="text-sm font-bold"> {essays[currentIndex]?.essayarea}
                              </p>
                              
                          </div>
                      </div>
                 
                      <div class="px-6 py-4 topic text-sm  bg-[#F8F0E3]">{essays[currentIndex]?.essaytopic}
                      </div>
                  </div><a href="" on:click|preventDefault={prevEssay}>
                    <img
                    src="/img/doublearrowright.svg"
                    class="w-8 h-8 ml-4 imgshadowed"
                    alt="Icon"
                    />
