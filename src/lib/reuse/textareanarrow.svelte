<script>
  ...
  <div class="flex flex-col items-center w-11/12 mt-4">
    <p class="text-sm font-semibold tracking-wide text-neutral-800">{title}</p>
    <div class="relative w-full mx-1 md:min-w-[600px] max-w-[650px]">
      {#if showWordCount}
  ...
  class="w-full px-2 py-1 pl-8 {bgcolor} text-neutral-800 {border} border-neutral-800 resize-y  transition-all duration-200 ease-in-out focus:border-red-800 focus:outline-none focus:scale-102 rounded text-sm z-10 relative"
  style="height: 100px; min-height: {minHeight};"
  id={id}
